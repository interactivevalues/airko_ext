...
$LANG->includeLLFile('EXT:tw_ersatzteilservice_import/mod1/locallang.xml');
...
/**
 * Module 'AIRKO Import' for the 'tw_ersatzteilservice_import' extension.
 *
 * @author	Thomas Wegerer <mail@thomaswegerer.at>
 * @package	TYPO3
 * @subpackage	tx_twersatzteilserviceimport
 */
class  tx_twersatzteilserviceimport_module1 extends t3lib_SCbase {
				...
				{
					$fp = fopen($this->uploadfile,"r"); unset($i); 
				...
				function update_insert()
				{
					...
					else if(t3lib_div::_GP("tbl") == "ersatzteil")
					{
					...
					$fp = fopen(t3lib_div::_GP("uploadfile"),"r"); unset($i);
					...
					for($i=1;$i < sizeof($arrZeile); $i++)
					{
						...
						for($x=0; $x < $arrSize; $x++)
						{
							...
							if(trim($arrFeldname[$x]) == "ersatzteilbezeichnung") //Ersatzteilbestellnummer durch ID des Ersatzteils ersetzen
							{
								...
								'bestellnummer = '.$ersatzteilBestellnummer,  #where
								$groupBy='',
								$orderBy='uid',
								$limit='1');
								...
								$field_values[trim($arrFeldname[$x])] = $ersatzteilUID; //Wert überschreiben
							}
							...
							if(trim($arrFeldname[$x]) == "produkt") //Produktbestellnummer durch ID des Produkts ersetzen
							{
								...
								'bestellnummer = '.$produktBestellnummer,  #where
								$groupBy='',
								$orderBy='uid',
								$limit='1');
								...
								$field_values[trim($arrFeldname[$x])] = $produktUID; //Wert überschreiben
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tw_ersatzteilservice_import/mod1/index.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tw_ersatzteilservice_import/mod1/index.php']);
}
...
// Make instance:
$SOBE = t3lib_div::makeInstance('tx_twersatzteilserviceimport_module1');
$SOBE->init();
